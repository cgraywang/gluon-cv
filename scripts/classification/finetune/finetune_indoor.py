...
def parse_opts():
    parser = argparse.ArgumentParser(description='Transfer learning on MIT Indoor 67 dataset',
...
def train(train_path, val_path):
    ...
    with finetune_net.name_scope():
        finetune_net.fc = nn.Dense(classes)
    ...
    trainer = gluon.Trainer(finetune_net.collect_params(), 'sgd', {
        'learning_rate': opts.lr, 'momentum': momentum, 'wd': wd})
    metric = mx.metric.Accuracy()
